...
from GF import enter_a_binari_list
...
def comprovar_canvi_Byte_SUB(missatge, clau):
    M = int(missatge, 16) #String hexadecimal a enter per poder fer la XOR despres
    C = int(aes.encryption(missatge, clau), 16) #String hexadecimal a enter per poder fer la XOR despres

    for i in range(128): #El bloc es de 128
        M_i = M ^ (1 << (127 - i)) #Modifiquem nomes el bit i del bloc M
        M_i = '{:x}'.format(M_i) #Ho expresem com a string hexadecimal per tal de poder-ho encriptar
        while len(M_i) < 32:
            M_i = '0' + M_i
        C_i = int(aes.encryption(M_i, clau), 16) #String hexadecimal a enter per poder fer la XOR despres

        for j in range(i+1, 128):
            M_j = M ^ (1 << (127 - j)) #Modifiquem nomes el bit j del bloc M
            M_j = '{:x}'.format(M_j) #Ho expresem com a string hexadecimal per tal de poder-ho encriptar
            while len(M_j) < 32:
                M_j = '0' + M_j
            C_j = int(aes.encryption(M_j, clau), 16) #String hexadecimal a enter per poder fer la XOR despres

            M_ij = M ^ ((1 << (127 - i)) | (1 << (127 - j))) #Modifiquem nomes els bits i, j del bloc M
            M_ij = '{:x}'.format(M_ij) #Ho expresem com a string hexadecimal per tal de poder-ho encriptar
            while len(M_ij) < 32:
                M_ij = '0' + M_ij
            C_ij = int(aes.encryption(M_ij, clau), 16) #String hexadecimal a enter per poder fer la XOR despres

            if (C != (C_i ^ C_j ^ C_ij)): 
                return False #Sense el return state ha de retornar FALSE
    return True #Amb el return state ha de retornar TRUE      

def comprovar_efectes_ShiftRows_o_MixColumns(missatge, clau):
    for i in range(10): #Comprovem per a 10 missatges diferents
        M = int(missatge, 16) #String hexadecimal a enter per poder fer la XOR despres
        C = aes.encryption(missatge, clau)

        M_i = M ^ (1 << (127 - i)) #Modifiquem nomes el bit i del bloc M
        M_i = '{:x}'.format(M_i) #Ho expresem com a string hexadecimal per tal de poder-ho encriptar
        while len(M_i) < 32:
            M_i = '0' + M_i
        C_i = aes.encryption(M_i, clau)

        missatge = int(missatge, 16) - random.randrange(10, 50000) #Canviem el missatge
        missatge = '{:x}'.format(missatge) #Ho expresem com a string hexadecimal per tal de poder-ho encriptar
        while len(missatge) < 32:
            missatge = '0' + missatge

        print(C)
        print(C_i)
        diferencies = int(C,16) ^ int(C_i,16)
        diferencies = '{:x}'.format(diferencies) #Ho expresem com a string hexadecimal
        while len(diferencies) < 32:
            diferencies = '0' + diferencies
        print(diferencies)
        print('-' * 50)

def calcular_num_bits_canviats(C, C_i):
	diferencies = int(C,16) ^ int(C_i,16)
	list_diferencies_binari = enter_a_binari_list(diferencies)

	num_bits_canviats = 0
	for i in range(len(list_diferencies_binari)):
		if list_diferencies_binari[i] == 1:
			num_bits_canviats += 1
	return num_bits_canviats

def calcular_posicions_canviades(C, C_i):
	diferencies = int(C,16) ^ int(C_i,16)
	list_diferencies_binari = enter_a_binari_list(diferencies)
	...
	for i in range(len(list_diferencies_binari)):
		if list_diferencies_binari[i] == 1:
			posicions_canviades.append(i)

	return posicions_canviades

def generar_estadistica_canvi_missatge(missatge, clau):
	histograma_num_bits_canviats = []
	histograma_posicions_canviades = {}

	for i in range(128):
		histograma_posicions_canviades[i] = 0 #inicialitzem la llista a 0
	...
	for i in range(128):
		...
		num_bits_canviats = calcular_num_bits_canviats(C, C_i)
		posicions_canviades = calcular_posicions_canviades(C, C_i)

		histograma_num_bits_canviats.append(num_bits_canviats)

		for j in range(len(posicions_canviades)):
			posicio_canviada = posicions_canviades[j]
			histograma_posicions_canviades[posicio_canviada] += 1

	plt.title('Numero de bits canviats a cada modificacio')
	plt.bar(range(128), histograma_num_bits_canviats)
	plt.xlabel("Modificacions")
	plt.ylabel("Numero de bits canviats")
	plt.grid(True)
	plt.show()

	plt.title('Posicions canviades')
	plt.bar(histograma_posicions_canviades.keys(), histograma_posicions_canviades.values())
	plt.xlabel("Posicio del bit")
	plt.ylabel("Numero de canvis")
	plt.grid(True)
	plt.show()

def generar_estadistica_canvi_clau(missatge, clau):
	histograma_num_bits_canviats = []
	histograma_posicions_canviades = {}
	...
	C = aes.encryption(missatge, clau)
	K = int(clau, 16) #String hexadecimal a enter per poder fer la XOR despres

	for i in range(128):
		K_i = K ^ (1 << (127 - i)) #Modifiquem nomes el bit i del bloc K
		K_i = '{:x}'.format(K_i) #Ho expresem com a string hexadecimal per tal de poder-ho encriptar
		while len(K_i) < 32:
			K_i = '0' + K_i
		C_i = aes.encryption(missatge, K_i)
...
def trobar_K_amb_maxim_0_C(missatge, clau):
    cont_0 = 0
    criptograma_0 = 'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'
    clau_0 = 'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'
    ...
    for i in range(20000000):
        ...
        if contador > cont_0:
            ...
            criptograma_0 = C
            clau_0 = clau
            print('iteracio numero: ' + str(i))
            print('C: ' + str(criptograma_0))
            print('K: ' + str(clau_0))
            print('-' * 50)
        
        clau = int(clau, 16) - 1 #Canviem el la clau
        clau = '{:x}'.format(clau) #Ho expresem com a string hexadecimal per tal de poder-ho encriptar
        while len(clau) < 32:
            clau = '0' + clau
...
def trobar_M_amb_maxim_0_C(clau):
    C = '00000000000000000000000000000000'
    missatge = aes.decryption(C, clau)
...
if __name__ == '__main__':
    ...
    # Test vector 128-bit key
    #clau = '0425456799ABCBEFBEECBA9276583280' #String hexadecimal
    #missatge = '00112233445566778899AABBCCDDEEFF' #String hexadecimal

    #----------------------------------COMPROVACIO FUNCIO comprovar_canvi_Byte_SUB(missatge, clau)----------------------------
    #resultat = comprovar_canvi_Byte_SUB(missatge, clau)
    #print(resultat)

    #----------------------------------COMPROVACIO FUNCIO comprovar_efectes_ShiftRows_o_MixColumns(missatge, clau)-------------
    #comprovar_efectes_ShiftRows_o_MixColumns(missatge, clau)

    #----------------------------------COMPROVACIO FUNCIO generar_estadistica_canvi_missatge/clau(missatge, clau)-------------------
    #generar_estadistica_canvi_missatge(missatge, clau)
    #generar_estadistica_canvi_clau(missatge, clau)

    #----------------------------------COMPROVACIO FUNCIO trobar_K_amb_maxim_0_C(missatge, clau)-------------------
    #clau = 'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF' #String hexadecimal
    #trobar_K_amb_maxim_0_C(missatge, clau)

    #----------------------------------COMPROVACIO FUNCIO trobar_M_amb_maxim_0_C(missatge, clau) donada un clau-------------------
    #clau = '0123456789ABCDEFFEDCBA9876543210' #String hexadecimal
    #trobar_M_amb_maxim_0_C(clau)

    #----------------------------------COMPROVACIO FUNCIO trobar_M_amb_maxim_0_C(missatge, clau)-------------------
    #trobar_M_amb_maxim_0_C(clau)

    key = '0425456799ABCBEFBEECBA9276583280' #String hexadecimal

    expanded_key = aes.key_handler(key, False) #Aplicar expansió de la clau  FALSE SI VOLEM ENCRIPTAR-TRUE SI VOLEM DESENCRIPTAR
    ...
    data = 'ACACACACACACACACACACACACACACACAC'

    state = aes.StateMatrix(data)

    print(state)

    #state = aes.SubBytes(state, False) #Aplicar SubBytes        FALSE SI VOLEM ENCRIPTAR-TRUE SI VOLEM DESENCRIPTAR
    #state = aes.ShiftRows(state, False) #Aplicar ShiftRows      FALSE SI VOLEM ENCRIPTAR-TRUE SI VOLEM DESENCRIPTAR
    state = aes.MixColumns(state, False) #Aplicar MixColumns    FALSE SI VOLEM ENCRIPTAR-TRUE SI VOLEM DESENCRIPTAR
    print(state)
    state = aes.AddRoundKey(state, expanded_key[2]) #Aplicar AddRoundKey agafant la tercera subclau generada (contant la clau inicial)
